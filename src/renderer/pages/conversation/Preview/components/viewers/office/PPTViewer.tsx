  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        throw new Error(
          jsonResp.to === "ppt-json" ? (jsonResp.result.error ?? "") : "",
        );
      } catch (err) {
        setState({
          status: "error",
          message:
            err instanceof Error ? err.message : t("preview.ppt.loadFailed"),
        });
      }
    };
        ...
        >
          <div className="flex-1 overflow-y-auto p-24px">
            ...
            {slide.texts.map((text, i) => (
              <p
                key={i}
                className="text-14px text-t-secondary leading-relaxed mb-6px"
              >
                {text}
              </p>
            ))}
